...
public class FloatUtil {
	...
	protected static BigInteger sqrt(BigInteger number, BigInteger guess) {
		...
		// BigInteger result =
		// number.divide(guess).add(guess).divide(BigIntegerTWO);
	...
	public static BigDecimal sqrt(BigDecimal number, BigDecimal guess,
		...
		while (result.compareTo(guess) != 0) {
			...
			result = number.divide(guess, rounding).add(guess)
					.divide(BigDecimalTWO, rounding);
			// handle flip flops
			if (result.equals(flipB))
				return flipA;
